...
Collection literals introduce a new terse syntax, `[e1, e2, e3, etc]`, to create common collection values in target-typing scenarios.  Inlining other collections into these values is possible using a spread operator `..` like so: `[e1, .. c2, e2, .. c2]`.  A `[k1: v1, .. d]` form is also supported for creating dictionaries.
...
1. If the literal contains no `dictionary_element` but has `expression` and

### Known length translation

1. For a *known length* literal `[e1, k1:v1, .. s1, e2, k2:v2, .. s2, etc]` (with no `dictionary_element`), the translation first starts with the following:
...
2. Given a target type `T` for that literal:
    ...
    - If `T` supports [collection initializers](https://github.com/dotnet/csharplang/blob/main/spec/expressions.md#collection-initializers), then:
        ...
        - if the type `T` contains an accessible constructor with a single parameter '`int capacity`', then the literal is translated as:
            ...
            Unresolved questions.  Optimizations could occur by using methods like `.CopyTo`.  Should we explicitly decide if these will or won't be called?  Or should we spec such that we leave the door open for the implementation to choose to call that if it so decides?
