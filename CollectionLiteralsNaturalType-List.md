...
1. `T` inference can happen with the entire method and inheritance surface area of `List<T>`.  This is quite a large surface area.  We could consider limiting to just core methods like `Add/AddRange/this[]`, and/or if the variable is target typed to `List<T>/I<T>`.
