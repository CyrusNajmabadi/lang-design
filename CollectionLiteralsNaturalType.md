...
1. When 'v' is target-typed to some concrete `X<A>` instantiation, then `T` gets an upper and lower bound of `A`.  For example an inferred generic type `List<>` target type of `List<int>` infers `int` for `T` 
    1. When 'v' is target-typed to some concrete *invariant* `I<A>` interfaces implemented by `X<A>` (e.g. `IList<int>`), then `T` gets an upper and lower bound of `X`.
