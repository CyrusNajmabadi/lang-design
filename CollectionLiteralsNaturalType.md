...
1. Support broad number of use cases in a natural/intuitive fashsion.
...
The ``anonymous_list`<T>`` type has *roughly* the same semantics as the following type, with several important distinctions outlined below:
...
1. While this is a ref-type, it can be used in contexts where ref-types are normally not allowed (like an async method).
    ...
    1. When not safe to use a `Span<T>` the anonymous_list is created with the constructor that takes an array.  This value then only points at heap data, and can be used safely.
